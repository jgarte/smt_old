...
(defun inverse-toplvl-scale-posidims! (xmlelem)
  (dolist (attr-val (xml-base::element-attributes xmlelem))
...
(defun replace-with-transform! (xml-elem)  
  (multiple-value-bind (trns indxd) (svg::extract-transformations xml-elem)
    (when trns
      (setf (xml-base::element-attributes xml-elem) (set-difference (xml-base::element-attributes xml-elem) trns))
      (push (cons "transform"
		  (let ((s ""))
	    ...
	    (xml-base::element-attributes xml-elem)))))
